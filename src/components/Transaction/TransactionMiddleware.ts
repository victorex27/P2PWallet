...
export const GetSenderMiddleware = async (
    req: UserRequest,
    res: Response,
    next: NextFunction
) => {
    try {
        ...
        if (error instanceof Error) next(AuthenticationError(error.message))
        else next(AuthenticationError('Invalid token'))
    }
}

export const doesUserHaveSufficientBalance = async (
    req: UserRequest,
    res: Response,
    next: NextFunction
) => {
...
export const isUserEmailTheSameWithRecipient = async (
    req: UserRequest,
    res: Response,
    next: NextFunction
) => {
